...
void Port_RefreshPortDirection(void)
{
	...
	if (FALSE == error)
	{
		...
		{
			switch (Port_Pins[Configured_Pins_Index].Port_RegNumber)
			{
				case 0:
			...
			if (PORT_PIN_OUT == Port_Pins[Configured_Pins_Index].Port_PinDirection)
			{
				SET_BIT(*DirectionRegister_Ptr, Port_Pins[Configured_Pins_Index].Port_PinNumber);
			}
			else if (PORT_PIN_IN == Port_Pins[Configured_Pins_Index].Port_PinDirection)
			{
				CLEAR_BIT(*DirectionRegister_Ptr, Port_Pins[Configured_Pins_Index].Port_PinNumber);
			}
			else
			{
				/* No Action Required */
			}
			Configured_Pins_Index++;
		} /* while (PORT_CONFIG_PORT_PINS > Configured_Pins_Index) */
	}
}
